...
FText FDependencyUtils::GetDependencyDescriptionText(const FComponentDependency& Dependency)
{
    ...
    const bool needsTag{
        Dependency.Position == EComponentDependencyPosition::AnyOnActorWithTag || Dependency.Position == EComponentDependencyPosition::ChildWithTag
    };
    ...
    FText dependencyText{ FText::Format(
    NSLOCTEXT("CompDep", "DependencyText", "{0}{1}Dependency{2}"),
    {
...
TArray<UActorComponent*> FDependencyUtils::GetAllBlueprintComponentsByName(const UBlueprintGeneratedClass* BlueprintClass)
{
    ...
    {
        UActorComponent* typedComponent{ Cast<UActorComponent>(InnerComp) };
        if (typedComponent)
        {
    ...
    for (const UBlueprintGeneratedClass* currentClass : blueprintClassHierarchy)
    {
        ...
        if (currentClass->SimpleConstructionScript)
        {
            ...
            {
                if (UActorComponent * compTemplate{ node->GetActualComponentTemplate(const_cast<UBlueprintGeneratedClass*>(currentClass)) })
                {
